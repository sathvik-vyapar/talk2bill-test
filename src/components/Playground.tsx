    ...
    try {
      const modelResponses = await Promise.all(
        selectedModels.map(async (modelId) => {
          try {
            const response = await fetch('https://analytics-staging.vyaparapp.in/api/ps/extract-json-text', {  // You'll need to set up this endpoint
              method: 'POST',
              headers: {
            ...
            return {
              modelId,
              modelName: availableModels.find(m => m.id === modelId)?.name || '',
              response: JSON.stringify(data.invoice, null, 2),
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Textarea
              placeholder="Enter your input here (e.g., 'Generate a GST invoice for a Kirana store in Bihar')"
              value={input}
